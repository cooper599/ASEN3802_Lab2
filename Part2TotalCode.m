...
for i = 1:length(a)
    ...
    % Model II Plot/Task 4
    Mexp = slopes_0(i);
    T0 = intercepts_0(i);
    figure(); hold on; grid on;
    for j = 1:8
        % For scale factor did H-M instead M-H so (-1)^n could be used 
        % absorbed negative sign into expression
        Tana = rodTransientModel(thermocoupleLoc(j), t, L, alpha, H_IA, T0, N, (H_IA-Mexp));
        plot(t, Tana, 'LineWidth', 1.2);
        plot(t, Texpt(:,j), '--', 'LineWidth', 1.0);
    end
